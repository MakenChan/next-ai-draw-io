...
export async function POST(req:Request,{params}:{params:Promise<{id:string}>}){
 ...
 if(p.summaryText.trim()){try{
  const {model,providerOptions,headers}=getAIModel({provider:req.headers.get("x-ai-provider"),baseUrl:req.headers.get("x-ai-base-url"),apiKey:req.headers.get("x-ai-api-key"),modelId:req.headers.get("x-ai-model")})
