...
export default function ProjectDiagramEditor(){
 ...
 useEffect(()=>{load();loadVersions()},[p.diagramId])
 async function save(xml:string){setStatus("保存中...");const x=await fetch(`/api/project-diagrams/${p.diagramId}`,{method:"PATCH",headers:{"content-type":"application/json"},body:JSON.stringify({xml,source:"manual"})}).then(r=>r.json());if(x.diagram)setD(x.diagram);setStatus("已保存");loadVersions()}
 async function restore(v:number){const x=await fetch(`/api/project-diagrams/${p.diagramId}/versions`,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({version:v})}).then(r=>r.json());if(x.diagram){setD(x.diagram);ref.current?.load({xml:x.diagram.xml});loadVersions()}}
